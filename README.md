...
| `DEFAULT_DUE_TIME` | `09:00` | 只有日期没有时间的提醒、以及**全天日历事件**用它。建议 `23:59` —— 语义是「当天之内做完」，而 `09:00` 会让它一早就显示成已过期，全天事件还会被 `EXPIRE_HOURS` 在上午十点就划掉 |
| `INCLUDE_UNDATED` | `0` | 没有截止日期的提醒是否当作今天同步过去 |
| `SYNC_NEW_COMPLETED` | `0` | 是否把**已完成**的新任务也建到对面（默认不建，避免刷屏） |
| `DELETE_POLICY` | `apple-master` | 见下 |
| `CHECK_SOURCE_VISIBLE` | `1` | 删之前先确认那一项的来源这台读得到。`0` = 不检查，见下 |
...
### `CHECK_SOURCE_VISIBLE`

每条同步过去的待办都记着来源（提醒列表名 / 日历名）。默认在删之前会核对
**这台机器这次是不是真读到了那个来源** —— 没读到就不动它。

这是为多台电脑设计的：新装的第二台如果还没加 UMD 日历账号，它读不到那些
会议，但不会因此把你在第一台同步上去的日程删掉。代价是那台上真删掉的东西
也不会被镜像过去，得由能看到来源的机器来处理。

```env
CHECK_SOURCE_VISIBLE=0
```

关掉后任何一台发现某项不在了就直接删。**前提是两台看到的提醒列表和日历
完全一致** —— 否则残缺的那台会把另一台的内容清掉（这坑真踩过，丢了 4 条）。
关掉时启动日志会一直提示，免得以后东西没了想不起来原因。

关掉的只是「来源可见性」这一层。「整个来源拉取失败就不删」和「读不到
Zectrix 待办列表就中止」这两层始终有效 —— 那是「连不上」，不是
「这个日历不在我账号里」，在那种状态下删除是纯粹的数据损失。
...
- 时区自动处理；只有日期没有时间的提醒、以及全天日历事件，都按 `DEFAULT_DUE_TIME` 处理。
